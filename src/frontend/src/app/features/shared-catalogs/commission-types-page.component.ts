...
@Component({
  selector: 'app-commission-types-page',
  ...
  template: `
    <section class="page-shell">
      <article class="hero-card">
        ...
        <h2>Catalogo de tipos de comision</h2>
        <p>
          Base compartida para clasificar comisiones futuras sin acoplarla todavia a un modulo
      ...
      <div class="page-grid">
        <article class="form-card">
          <div class="card-header">
            <div>
              <h3>Alta minima</h3>
              <p>Se registran solo codigo, nombre, descripcion y orden.</p>
            </div>
          </div>
          ...
          <form class="form-grid" [formGroup]="form" (ngSubmit)="submit()">
            <label>
              <span>Codigo</span>
              <input type="text" formControlName="code" placeholder="COMMERCIAL" />
            </label>

            <label>
              <span>Nombre</span>
              <input type="text" formControlName="name" placeholder="Comercial" />
            </label>

            <label class="full-width">
              <span>Descripcion</span>
              <textarea formControlName="description" rows="4" placeholder="Uso base del tipo de comision"></textarea>
            </label>
        ...
        <article class="list-card">
          ...
          @if (loadError()) {
            ...
            <p class="empty-state">Cargando tipos de comision...</p>
          } @else {
...
export class CommissionTypesPageComponent {
  ...
  protected async submit() {
    ...
    try {
      const item = await firstValueFrom(
        this.sharedCatalogsService.createCommissionType({
      ...
      this.resetForm();
      this.submitSuccess.set('Tipo de comision registrado correctamente.');
    } catch (error) {
      this.submitError.set(getApiErrorMessage(error, 'No fue posible registrar el tipo de comision.'));
    } finally {
  ...
  private async load() {
    ...
    try {
      this.items.set(await firstValueFrom(this.sharedCatalogsService.getCommissionTypes()));
      this.form.patchValue({ sortOrder: this.nextSortOrder() });
    } catch (error) {
      this.loadError.set(getApiErrorMessage(error, 'No fue posible cargar el catalogo de tipos de comision.'));
    } finally {
