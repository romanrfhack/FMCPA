...
public sealed class AuthorizationRegressionTests : IClassFixture<AuthorizationRegressionWebApplicationFactory>
{
    ...
    private static IReadOnlyList<EndpointAuthorizationCase> ProtectedEndpoints()
    {
        ...
        [
            ...
            new("Donation documentary status read", HttpMethod.Get.Method, $"/api/donations/{MissingEntityId}/documentary-status", allRoles, []),
            new("Donation transparency report read", HttpMethod.Get.Method, $"/api/donations/{MissingEntityId}/transparency-report", allRoles, []),
            ...
            new("Financials context card read", HttpMethod.Get.Method, "/api/financials/context-card?financialName=F&institutionOrDependency=D&placeOrStand=S", allRoles, []),
