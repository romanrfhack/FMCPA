...
curl -i "http://127.0.0.1:5080/api/financials/context-card?financialName=Financiera%20Local&institutionOrDependency=Direccion%20local&placeOrStand=Stand%202" -H "Authorization: Bearer ${ADMIN_TOKEN}"
# Debe responder 200 con `resolution`, `renewalChainSummary`, `creditSummary`, `commissionSummary` y acciones disponibles. Con vigente debe incluir `CAPTURE_CREDIT`, `VIEW_CURRENT_PERMIT` y `VIEW_CHAIN`.
curl -i "http://127.0.0.1:5080/api/financials/context-card?financialName=Financiera%20Local&institutionOrDependency=Direccion%20local&placeOrStand=Stand%20inexistente" -H "Authorization: Bearer ${ADMIN_TOKEN}"
# Debe responder 200 con `suggestionCode=CREATE_NEW_PERMIT`, sin resumen de cadena y con accion `CREATE_PERMIT`.
curl -i "http://127.0.0.1:5080/api/financials/current-permit?financialName=Financiera%20Local&institutionOrDependency=Direccion%20local&placeOrStand=Stand%202" -H "Authorization: Bearer ${ADMIN_TOKEN}"
...
- Si `GET /api/financials/context-card` no muestra cadena, creditos o comisiones, revisar primero `resolution.suggestionCode`: para `CREATE_NEW_PERMIT` no hay antecedente y esos resumenes quedan vacios; para vigente, historico o terminal debe existir `currentPermit` o `lastKnownPermit` y la ficha reutiliza `/renewal-chain`.
- Si la ficha muestra acciones, recordatorio operativo: son accesos de UI derivados de `suggestionCode`; las mutaciones reales siguen validando permisos y estado en `POST /api/financials`, `POST /api/financials/{permitId}/renew` y `POST /api/financials/{permitId}/credits`.
